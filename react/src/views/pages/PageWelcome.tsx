...
import { Coin, Login, Qiita, Weather } from 'views/components/service'
...
const qiita = [
  ...
  },
  {
    ...
    description:
      'ここ 1 年で作成・更新された記事を[「"golang" ポインタに値をセットする int」でググって](https://www.google.com/search?q=%22golang%22+%E3%83%9D%E3%82%A4%E3%83%B3%E3%82%BF%E3%81%AB%E5%80%A4%E3%82%92%E3%82%BB%E3%83%83%E3%83%88%E3%81%99%E3%82%8B+int\u0026tbs=qdr:y)も、わかりやすいタイトルがヒットしなかったので、自分のググラビリティとして。\n\n## TL; DR \u003csub\u003e\u003csup\u003e\u003csub\u003e\u003csup\u003e（今北産業）\u003c/sup\u003e\u003c/sub\u003e\u003c/sup\u003e\u003c/sub\u003e\n\n```go:変数の前に*を付ける\nfunc resetToTen(i *int) {\n\t*i = 10 // \u003c- ここ\n}\n```\n```go:動くサンプル\npackage main\n\nimport "fmt"\n\nfunc main() {\n\ti := 100\n\n\tresetToTen(\u0026i)\n\n\tfmt.Println(i)\n}\n\nfunc resetToTen(i *int) {\n\t*i = 10\n}\n// Output: 10\n```\n- [オンラインで動作をみる](https://go.dev/play/p/vlQgqqLtcMR) @ Go Playground\n\n## 参考文献\n\n- [Go で関数に渡したポインタの値を変更するには](https://qiita.com/m_green14/items/1efe3f887aee082ba4ef) @ Qiita\n  - 「`\u0026` や `*` をどうつけたらいいか、よくわからなくなってしまいがち」な人は必読。\n\n',
  },
  {
    title: 'zinのLogin',
  ...
  {
    title:
      '[GCP] Container failed to start. Failed to start and then listen on the port defined by the PORT environment variable.',
...
const coin = {
  date: '2021-11-29T08:33:00Z',
  jpyRate: 57451.8033,
  usdRate: 6520930.4606,
}
